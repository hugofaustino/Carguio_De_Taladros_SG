...
const chkAceiteResidual = document.getElementById("chkAceiteResidual");
...
const COMPOSICION_MEZCLAS = {
  "EMG 55G": { emulsion: 0.5, nitrato: 0.46, combustible: 0.04, gasificante: 0.7, kallpex: 0 },
  "EMG 55G K5": { emulsion: 0.5, nitrato: 0.46, combustible: 0.04, gasificante: 0.7, kallpex: 0.025 },
  "EMG 55G K10": { emulsion: 0.5, nitrato: 0.46, combustible: 0.04, gasificante: 0.7, kallpex: 0.05 },
  "EMG 73G": { emulsion: 0.7, nitrato: 0.276, combustible: 0.024, gasificante: 0.7, kallpex: 0 },
  "EMG 73G K5": { emulsion: 0.7, nitrato: 0.276, combustible: 0.024, gasificante: 0.7, kallpex: 0.035 }
};

const PORCENTAJE_ACEITE_RESIDUAL = 0.30;
const PORCENTAJE_DIESEL_CON_ACEITE = 0.70;
...
chkAceiteResidual?.addEventListener("change", () => {
  actualizarConsumoInsumos();
});
...
function construirEncabezadoTablaConFiltros(data, columnas) {
  ...
  columnas.forEach(col => {
    ...
    thTitulo.textContent = col.titulo;
...
function actualizarConsumoInsumos() {
  ...
  const usaAceiteResidual = !!chkAceiteResidual?.checked;
  const resultado = calcularConsumoInsumos(dataAnalisis, usaAceiteResidual);

  renderConsumoInsumos(resultado, usaAceiteResidual);
}

function calcularConsumoInsumos(data, usaAceiteResidual) {
  const porMezcla = {};
  ...
  data.forEach(fila => {
    ...
    const mezclaPrincipal = normalizarNombreMezcla(fila.Bulk || fila.Tipo_Mezcla);
    if (mezclaPrincipal && porMezcla[mezclaPrincipal]) {
      porMezcla[mezclaPrincipal].taladrosSet.add(fila.ID || fila.Label || fila.Taladro);
    }
  });

  Object.values(porMezcla).forEach(item => {
    const composicion = COMPOSICION_MEZCLAS[item.mezcla];
    ...
    const taladros = item.taladrosSet.size;
    ...
    item.gasificante = taladros * composicion.gasificante;

    if (usaAceiteResidual) {
      item.aceiteResidual = item.combustibleTotal * PORCENTAJE_ACEITE_RESIDUAL;
      item.diesel = item.combustibleTotal * PORCENTAJE_DIESEL_CON_ACEITE;
    } else {
      item.aceiteResidual = 0;
      item.diesel = item.combustibleTotal;
    }
  });
...
function normalizarNombreMezcla(nombre) {
  ...
  if (COMPOSICION_MEZCLAS[limpio]) return limpio;

  const encontrado = Object.keys(COMPOSICION_MEZCLAS).find(nombreMezcla =>
...
function renderConsumoInsumos(resultado, usaAceiteResidual) {
  ...
  resumenConsumoTexto.innerHTML = `
    ...
    Combustible: ${usaAceiteResidual ? "70% diesel / 30% aceite residual" : "100% diesel"}
  `;
